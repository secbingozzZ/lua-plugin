...
local redis_auth = "your_password"  -- Redis 服务器的认证密码
...
local function is_ip_in_blacklist(ip)
    local res, err = client:hexists("blacklist", ip)
    if err then
...
-- 如果 IP 在黑名单中，禁止访问
if is_ip_in_blacklist(clientIp) then
    ngx.exit(ngx.HTTP_FORBIDDEN)
    close_redis(client)
    return
end
...
-- 查询 IP 信誉值
local res, err = httpc:request_uri("http://127.0.0.1:9898/get_reputation", {
    method = "GET",
    query = { ip = clientIp }
})
...
-- 确保 res 不为 nil
if not res then
    errlog("failed to request reputation check: " .. (err or "unknown error"))
    close_redis(client)
    return
end
...
if res.status ~= 200 then
    errlog("invalid response status: " .. res.status)
    close_redis(client)
    return
end

local cjson = require "cjson"
local reputation_data, err

-- 使用 pcall 捕获 cjson.decode 的错误
local success, decode_err = pcall(function()
    reputation_data = cjson.decode(res.body)
end)

-- 如果解码失败或数据不正确，默认允许访问
if not success or not reputation_data or type(reputation_data) ~= "table" or not reputation_data.reputation_number then
    errlog("invalid reputation data received or decoding failed: " .. (decode_err or "unknown error"))
    close_redis(client)
    return
end

local reputation_score = tonumber(reputation_data.reputation_number)

-- 如果信誉值不存在或无效，默认允许访问
if not reputation_score then
    errlog("invalid reputation score received")
    close_redis(client)
    return
end
